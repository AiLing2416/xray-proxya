...
var presetsListCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		for i, mode := range cfg.ActiveModes {
			status := "OFF"; if mode.Enabled { status = "ON" }
			extra := mode.SNI; if mode.Path != "" { extra = mode.Path }
...
var presetsSetCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if id < 1 || id > len(cfg.ActiveModes) { fmt.Printf("❌ Invalid ID: %d\n", id); return }

		idx := id - 1
		m := &cfg.ActiveModes[idx]

		if presetOff { m.Enabled = false }
		if presetOn { m.Enabled = true }
		if presetPort > 0 { m.Port = presetPort }
		if presetRegen { m.RegenFlag = true }
